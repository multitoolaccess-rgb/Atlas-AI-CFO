...
Skip behavior: when ``.venv/bin/python`` is missing (fresh clone, no
...
def pytest_collection_modifyitems(config, items: list[pytest.Item]) -> None:
    """Skip ONLY the slow e2e cold-boot when the project .venv is missing.

    v1 of this conftest skipped every test in ``tests/`` when
    ``.venv/bin/python`` was absent. But the unit tests in
    ...
    skip_marker = pytest.mark.skip(
        reason=(
            "Project .venv missing — slow e2e cold-boot requires running "
