...
describe('information architecture activation contract', () => {
  it('activates Home, Money, and the complete Wealth wave', () => {
    ...
    expect(PROPOSED_NAVIGATION.flatMap((group) => group.destinations).filter((destination) => destination.activated).map((destination) => destination.id)).toEqual(['mission-control', 'cash-flow', 'plan', 'wealth', 'portfolio', 'goals', 'decisions', 'market-intelligence', 'scenario-lab', 'data-connections', 'settings', 'help'])
