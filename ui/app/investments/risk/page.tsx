...
function BaselineSummary({ baseline }: { baseline: PortfolioBaseline }) {
  ...
  return <>
    ...
    <section className="mt-4 card p-5" aria-labelledby="positions-heading"><h2 id="positions-heading" className="font-semibold text-primary">Position coverage</h2><p className="mt-1 text-sm text-secondary">Account names and account identifiers are intentionally omitted from this projection.</p><div className="mt-4 overflow-x-auto rounded-lg border border-[var(--border-subtle)]"><table className="w-full min-w-[44rem] text-left text-sm"><caption className="sr-only">Server-owned portfolio position coverage</caption><thead className="bg-[var(--surface-ambient)] text-xs text-tertiary"><tr><th scope="col" className="px-4 py-3 font-medium">Security</th><th scope="col" className="px-4 py-3 font-medium">Value</th><th scope="col" className="px-4 py-3 font-medium">Exposure</th><th scope="col" className="px-4 py-3 font-medium">Value state</th><th scope="col" className="px-4 py-3 font-medium">Identity</th><th scope="col" className="px-4 py-3 font-medium">Source</th></tr></thead><tbody className="divide-y divide-[var(--border-subtle)]">{baseline.positions.map((position) => <tr key={position.position_id}><th scope="row" className="px-4 py-3 font-medium text-primary">{position.security.symbol ?? 'Unresolved security'}</th><td className="px-4 py-3 font-mono text-primary">{position.market_value === null ? 'Unavailable' : `${position.currency ?? 'Unknown'} ${position.market_value}`}</td><td className="px-4 py-3 font-mono text-primary">{position.exposure_percentage === null ? 'Unavailable' : `${position.exposure_percentage}%`} <span className="ml-1 align-middle"><StateLabel state={position.exposure_state} /></span></td><td className="px-4 py-3"><StateLabel state={position.market_value_state} /></td><td className="px-4 py-3"><StateLabel state={position.security.state} /></td><td className="px-4 py-3 font-mono text-xs text-tertiary">{position.source_id}</td></tr>)}</tbody></table>{baseline.positions.length === 0 && <p className="p-5 text-sm text-secondary">No active positions are available for the current baseline.</p>}</div></section>
