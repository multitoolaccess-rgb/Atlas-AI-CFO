...
export default function InvestmentDiscoveryPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true); setError(null); setComparison(null); setDetail(null)
  ...
  const openDetail = async (item: DiscoveryCandidate) => {
    try { setDetail(await investmentDiscovery.detail(universe, item.candidate_id)) }
    catch { setError('Candidate detail is unavailable without a validated server response.') }
  }
  ...
  return <PageLayout><div className="min-w-0 max-w-full overflow-x-hidden [word-break:break-word]" style={{ maxWidth: '100vw' }}>
    ...
    {loading ? <p className="card p-6 text-sm text-secondary" role="status">Loading server-owned opportunities…</p> : items.length === 0 ? <section className="card p-6"><h2 className="font-semibold text-primary">No candidates in this universe</h2><p className="mt-2 text-sm text-secondary">The selected universe has no currently available candidate projection.</p></section> : <><div className="mb-3 flex items-center justify-between gap-3"><p className="text-sm text-secondary">{items.length} candidate{items.length === 1 ? '' : 's'} · select 2–10 to compare</p><button type="button" disabled={selected.length < 2} onClick={() => void compare()} className="btn-primary inline-flex min-h-11 items-center gap-2 px-3 text-sm disabled:opacity-50"><GitCompareArrows className="h-4 w-4" aria-hidden="true" />Compare selected</button></div><div className="grid min-w-0 gap-3 md:grid-cols-2 lg:grid-cols-3">{items.map((item) => <article key={item.candidate_id} className="card min-w-0 p-4"><div className="flex items-start justify-between gap-3"><div><h2 className="font-mono text-lg font-semibold text-primary">{item.security.symbol ?? 'Unnamed security'}</h2><p className="mt-1 max-w-full break-all text-xs text-tertiary">{item.security.security_id}</p></div><button type="button" onClick={() => toggle(item.candidate_id)} aria-label={`${selected.includes(item.candidate_id) ? 'Remove' : 'Add'} ${item.security.symbol ?? item.candidate_id} from comparison`} className="flex h-9 w-9 items-center justify-center rounded-md border border-[var(--border-subtle)]">{selected.includes(item.candidate_id) ? <Check className="h-4 w-4 text-[var(--success-600)]" aria-hidden="true" /> : <span className="text-tertiary">+</span>}</button></div><p className="mt-4 text-sm text-secondary">{item.reason}</p><button type="button" onClick={() => void openDetail(item)} className="mt-3 text-left text-xs font-medium text-[var(--accent-primary)] underline underline-offset-2">View server detail</button><dl className="mt-4 space-y-2 text-xs"><div className="flex justify-between gap-3"><dt className="text-tertiary">Status</dt><dd className="text-primary">{item.status}</dd></div><div className="flex justify-between gap-3"><dt className="text-tertiary">As of</dt><dd className="text-primary">{new Date(item.as_of).toLocaleString()}</dd></div><div className="flex justify-between gap-3"><dt className="text-tertiary">Data state</dt><dd className="text-primary">{item.freshness}</dd></div></dl></article>)}</div></>}
    {detail && <section className="card mt-6 p-5" aria-label="Candidate detail"><div className="flex items-center justify-between"><h2 className="text-lg font-semibold text-primary">{detail.security.symbol ?? 'Candidate detail'}</h2><button type="button" onClick={() => setDetail(null)} aria-label="Close candidate detail"><X className="h-4 w-4" aria-hidden="true" /></button></div><dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2"><div><dt className="text-tertiary">Universe</dt><dd className="mt-1 text-primary">{detail.universe}</dd></div><div><dt className="text-tertiary">Source</dt><dd className="mt-1 text-primary">{detail.source}</dd></div><div><dt className="text-tertiary">As of</dt><dd className="mt-1 text-primary">{new Date(detail.as_of).toLocaleString()}</dd></div><div><dt className="text-tertiary">Methodology</dt><dd className="mt-1 text-primary">{detail.methodology_version}</dd></div></dl><p className="mt-4 text-sm text-secondary">No financial metric is displayed unless the server provides an explicitly comparable observation.</p></section>}
