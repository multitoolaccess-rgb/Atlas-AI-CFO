...
def calculate_quant_research(security: SecurityIdentity, points: Sequence[PriceSeriesPoint], *, as_of: datetime, lookback: int = 5, risk_free_rate: Decimal | None = None, benchmark: Sequence[PriceSeriesPoint] | None = None) -> QuantResearch:
    ...
    cumulative = (closes[-1] / closes[0]) - Decimal(1) if len(closes) >= 2 else None
    ...
    if benchmark is not None:
        ...
        if len(benchmark) >= 2 and tuple(point.timestamp for point in benchmark[-len(points):]) == tuple(point.timestamp for point in points[-len(benchmark):]):
            ...
            benchmark_id = benchmark[0].source_observation_hash
        else:
            metrics.append(_metric("beta", None, QuantState.UNAVAILABLE, points, lookback, "ratio"))
    payload = {"security": security.model_dump(mode="json"), "as_of": as_of.astimezone(UTC).isoformat(), "metrics": [metric.model_dump(mode="json") for metric in metrics], "benchmark_security_id": benchmark_id, "risk_free_rate": str(risk_free_rate) if risk_free_rate is not None else None, "source_observation_hashes": hashes, "methodology_version": "quant-research/v1"}
    digest = hashlib.sha256(json.dumps(payload, sort_keys=True, separators=(",", ":")).encode()).hexdigest()
    return QuantResearch(security=security, as_of=as_of.astimezone(UTC), metrics=tuple(metrics), benchmark_security_id=benchmark_id, risk_free_rate=str(risk_free_rate) if risk_free_rate is not None else None, source_observation_hashes=hashes, research_hash=digest)
