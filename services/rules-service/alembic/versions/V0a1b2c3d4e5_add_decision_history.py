...
def _guards() -> None:
    ...
    if bind.dialect.name == "sqlite":
        ...
        WHEN length(NEW.id)!=36 OR NEW.id!=lower(NEW.id) OR NEW.id NOT GLOB '????????-????-????-????-????????????'
          ...
          OR NEW.event_action NOT IN ('recorded','corrected','evaluated') OR (NEW.event_action='evaluated' AND NEW.outcome_evaluation_id IS NULL) OR (NEW.event_action IN ('recorded','corrected') AND NEW.outcome_evaluation_id IS NOT NULL) OR NEW.actor_scope!='owner' OR NEW.policy_result!='recorded'
        ...
        IF length(NEW.id)!=36 OR NEW.id!=lower(NEW.id) OR NEW.id !~ '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$' OR NEW.correlation_hash !~ '^[0-9a-f]{64}$' OR NEW.event_action NOT IN ('recorded','corrected','evaluated') OR (NEW.event_action='evaluated' AND NEW.outcome_evaluation_id IS NULL) OR (NEW.event_action IN ('recorded','corrected') AND NEW.outcome_evaluation_id IS NOT NULL) OR NEW.actor_scope!='owner' OR NEW.policy_result!='recorded' THEN RAISE EXCEPTION 'decision audit values must be canonical'; END IF; RETURN NEW; END; $$ LANGUAGE plpgsql""")
