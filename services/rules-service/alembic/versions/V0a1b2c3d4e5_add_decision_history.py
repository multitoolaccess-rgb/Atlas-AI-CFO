...
def _guards() -> None:
    ...
    if bind.dialect.name == "sqlite":
        ...
        WHEN NEW.user_id != (SELECT user_id FROM goals WHERE id=NEW.goal_id)
          ...
          OR (NEW.supersedes_history_entry_id IS NOT NULL AND NEW.user_id != (SELECT user_id FROM decision_history_entries WHERE id=NEW.supersedes_history_entry_id))
        ...
        WHEN length(NEW.id)!=36 OR NEW.id!=lower(NEW.id) OR NEW.id NOT GLOB '????????-????-????-????-????????????'
          ...
          OR NEW.event_action NOT IN ('recorded','corrected') OR NEW.actor_scope!='owner' OR NEW.policy_result!='recorded'
        ...
        IF NEW.user_id != (SELECT user_id FROM goals WHERE id=NEW.goal_id) OR NEW.user_id != (SELECT user_id FROM recommendations WHERE id=NEW.recommendation_id) OR NEW.user_id != (SELECT user_id FROM decision_journal_entries WHERE id=NEW.decision_journal_entry_id) OR (NEW.supersedes_history_entry_id IS NOT NULL AND NEW.user_id != (SELECT user_id FROM decision_history_entries WHERE id=NEW.supersedes_history_entry_id)) THEN RAISE EXCEPTION 'decision history owner mismatch'; END IF; RETURN NEW; END; $$ LANGUAGE plpgsql""")
        ...
        IF length(NEW.id)!=36 OR NEW.id!=lower(NEW.id) OR NEW.id !~ '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$' OR NEW.correlation_hash !~ '^[0-9a-f]{64}$' OR NEW.event_action NOT IN ('recorded','corrected') OR NEW.actor_scope!='owner' OR NEW.policy_result!='recorded' THEN RAISE EXCEPTION 'decision audit values must be canonical'; END IF; RETURN NEW; END; $$ LANGUAGE plpgsql""")
