"""Append-only provenance for authoritative account-balance observations."""
...
class AccountBalanceObservation(Base):
    """Audit event binding an observation timestamp to a prior account state.

    The current balance is intentionally never stored here. ``precondition_hash``
    binds the event to the server-read account state, while ``last_sync`` on
    Account remains only the compatibility timestamp projection.
    """
